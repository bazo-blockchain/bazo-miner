...
func TestSlashingCondition(t *testing.T) {
	...
	forkBlock := newBlock([32]byte{}, [protocol.COMM_ENCODED_KEY_LENGTH]byte{}, 1)
	...
	// genesis <- forkBlock <- b
	b := newBlock(forkBlock.Hash, [protocol.COMM_ENCODED_KEY_LENGTH]byte{}, 2)
	...
	b2 := newBlock(forkBlock.Hash, [protocol.COMM_ENCODED_KEY_LENGTH]byte{}, 2)
