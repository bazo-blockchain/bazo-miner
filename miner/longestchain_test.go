...
func TestGetBlockSequences(t *testing.T) {
	...
	b2 := newBlock(b.Hash, [protocol.COMM_PROOF_LENGTH]byte{}, b.Height+1)
	...
	c := newBlock([32]byte{}, [protocol.COMM_PROOF_LENGTH]byte{}, 1)
	...
	//PoW needs lastBlock, have to set it manually
	lastBlock = c
	c2 := newBlock(c.Hash, [protocol.COMM_PROOF_LENGTH]byte{}, c.Height+1)
	...
	//PoW needs lastBlock, have to set it manually
	lastBlock = c2
	c3 := newBlock(c2.Hash, [protocol.COMM_PROOF_LENGTH]byte{}, c.Height+1)
	...
	lastBlock = c2
	c3 = newBlock(c2.Hash, [protocol.COMM_PROOF_LENGTH]byte{}, c2.Height+1)
	createBlockWithTxs(c3)
	finalizeBlock(c3)
