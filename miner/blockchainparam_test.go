...
func TestTargetHistory(t *testing.T) {
	...
	for cnt := 0; cnt < 10; cnt++ {
		tmpBlock = newBlock(tmpBlock.Hash, [protocol.COMM_ENCODED_KEY_LENGTH]byte{}, tmpBlock.Height+1)
	...
	tmpBlock = newBlock(blocks[len(blocks)-1].Hash, [protocol.COMM_ENCODED_KEY_LENGTH]byte{}, blocks[len(blocks)-1].Height+1)
	finalizeBlock(tmpBlock)
	validate(tmpBlock, false)
...
func TestTimestamps(t *testing.T) {
	...
	prevHash := [32]byte{}
	for cnt := 0; cnt < 0; cnt++ {
		b := newBlock(prevHash, [protocol.COMM_ENCODED_KEY_LENGTH]byte{}, 1)
