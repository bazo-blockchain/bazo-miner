...
func TestProofOfStake(t *testing.T) {
	...
	var prevProofs [][protocol.COMM_KEY_LENGTH]byte
...
func TestGetLatestProofs(t *testing.T) {
	...
	var proofs [][protocol.COMM_KEY_LENGTH]byte
	genesisCommitmentProof, _ := protocol.SignMessageWithRSAKey(&CommPrivKeyRoot, "0")
	proofs = append([][protocol.COMM_KEY_LENGTH]byte{genesisCommitmentProof}, proofs...)
	...
	b1 := newBlock([32]byte{}, [protocol.COMM_KEY_LENGTH]byte{}, 1)
	if err := finalizeBlock(b1); err != nil {
		t.Error("Error finalizing b1", err)
	}
	proofs = append([][protocol.COMM_KEY_LENGTH]byte{b1.CommitmentProof}, proofs...)
	validate(b1, false)

	b2 := newBlock(b1.Hash, [protocol.COMM_KEY_LENGTH]byte{}, b1.Height+1)
	if err := finalizeBlock(b2); err != nil {
		t.Error("Error finalizing b2", err)
	}
	validate(b2, false)
	proofs = append([][protocol.COMM_KEY_LENGTH]byte{b2.CommitmentProof}, proofs...)
