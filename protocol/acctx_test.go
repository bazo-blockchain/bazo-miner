...
func TestAccTxCreation(t *testing.T) {
	header := byte(0)
	fee := uint64(1)
	tx, newKey, _ := ConstrAccTx(header, fee, accA.Address, &RootPrivKey)
	...
	header = byte(1)
	fee = uint64(2)
	tx, newKey, _ = ConstrAccTx(header, fee, [64]byte{}, &RootPrivKey)
...
func TestAccTxHash(t *testing.T) {
	header := byte(0)
	fee := uint64(1)
	tx, _, _ := ConstrAccTx(header, fee, accA.Address, &RootPrivKey)
	...
	header = byte(1)
	fee = uint64(2)
	tx, _, _ = ConstrAccTx(header, fee, [64]byte{}, &RootPrivKey)
