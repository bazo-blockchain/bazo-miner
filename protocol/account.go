...
func NewAccount(address [64]byte, balance uint64, isRoot, isStaking bool, hashedSeed [32]byte) Account {
	newAcc := Account{
		address,
		balance,
		0,
...
func (acc *Account) Encode() (encodedAcc []byte) {
	...
	var isRootBuf, isStakingBuf [1]byte
	var txCntBuf, stakingBlockHeightBuf [4]byte

	var buf bytes.Buffer

	binary.Write(&buf, binary.BigEndian, acc.IsRoot)
	copy(isRootBuf[:], buf.Bytes())
	...
	copy(encodedAcc[77:78], isStakingBuf[:])
	copy(encodedAcc[78:110], acc.HashedSeed[:])
	copy(encodedAcc[110:114], stakingBlockHeightBuf[:])
...
func (*Account) Decode(encodedAcc []byte) (acc *Account) {
	...
	acc.IsRoot, _ = strconv.ParseBool(string(encodedAcc[76:77]))
	acc.IsStaking, _ = strconv.ParseBool(string(encodedAcc[77:78]))
	copy(acc.HashedSeed[:], encodedAcc[78:110])
	acc.StakingBlockHeight = binary.BigEndian.Uint32(encodedAcc[110:114])
...
func (acc Account) String() string {
	...
	return fmt.Sprintf("Hash: %x, Address: %x, TxCnt: %v, Balance: %v, IsRoot: %v, IsStaking: %v, HashedSeed: %x, StakingBlockHeight: %v", addressHash[0:12], acc.Address[0:8], acc.TxCnt, acc.Balance, acc.IsRoot, acc.IsStaking, acc.HashedSeed[0:8], acc.StakingBlockHeight)
}
