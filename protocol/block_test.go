...
func TestBlockCreation(t *testing.T) {
	...
	createdBlock := NewBlock(prevHash, seed, hashedSeed, height)
	...
	if !reflect.DeepEqual(createdBlock.Seed, seed) {
		t.Errorf("Balance does not match the given one: %v vs. %v", createdBlock.Seed, seed)
	}

	if !reflect.DeepEqual(createdBlock.HashedSeed, hashedSeed) {
		t.Errorf("Hashed seed does not match the given one: %v vs. %v", createdBlock.HashedSeed, hashedSeed)
	}
...
func TestBlockHash(t *testing.T) {
	...
	block := NewBlock(prevHash, seed, hashedSeed, height)
...
func TestBlockSerialization(t *testing.T) {
	...
	rand.Read(block.Seed[:])
	block.Height = uint32(randVar.Uint32())
	rand.Read(block.HashedSeed[:])
