...
func TestReadWriteDeleteTx(t *testing.T) {
	...
	loopMax := testsize
	for i := 0; i < loopMax; i++ {
		tx, _ := protocol.ConstrFundsTx(0x01, rand.Uint64()%100000+1, rand.Uint64()%10+1, uint32(i), accAHash, accBHash, &PrivKeyAccA, nil)
		WriteOpenTx(tx)
	...
	nullAddress := [64]byte{}
	for i := 0; i < 1000; i++ {
		tx, _, _ := protocol.ConstrAccTx(0, rand.Uint64()%100+1, nullAddress, &PrivKeyRoot)
		WriteOpenTx(tx)
	...
	for cnt := 0; cnt < loopMax; cnt++ {
		tx, _ := protocol.ConstrConfigTx(uint8(rand.Uint32()%256), uint8(rand.Uint32()%5+1), rand.Uint64()%2342873423, rand.Uint64()%1000+1, uint8(cnt), &PrivKeyRoot)
	...
	for cnt := 0; cnt < loopMax; cnt++ {
		...
		tx, _ := protocol.ConstrStakeTx(0, uint64(cnt), isStaking, [32]byte{}, accAHash, &PrivKeyAccA)
